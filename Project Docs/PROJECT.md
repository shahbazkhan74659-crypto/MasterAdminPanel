...
- Staging layer for data-management edits (**built as of [Phase 23], 2026-09-11**): edits made through the schema-driven Content Admin API path are held as drafts in AdminPanel (a `drafts` table in AdminPanel's own Postgres, a full shadow copy per record) rather than written straight to a target site's live database, with the split-pane UI (`Phase 22`) wired to a real staged-vs-live comparison and an explicit, confirm-gated "deploy" action that applies it — see `DECISIONS.md`'s "Staging layer for data-management edits, deployed to live on demand" and "Phase 23 implementation" entries. Deliberately does **not** apply to the SQL Console, which keeps its own existing read-only-by-default/confirm-to-write model. Currently wired end-to-end for one demo record only ([Phase 24] generalizes this to every record).
