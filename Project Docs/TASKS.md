...
[Phase 1] "Local Database Setup" — not yet started. **Split into 1a/1b on 2026-09-08 (Admin's direct command):** [Phase 1a] "Local Postgres for the App Itself" (a dedicated local Postgres instance for AdminPanel's own data — login account, secrets — separate from any managed-site database) and [Phase 1b] "All Database Engine Types Setup" (local Postgres, MySQL, and SQLite as target/managed databases, since the panel needs to support whichever engine a managed site actually runs — e.g. Portfolio → Postgres, TS Library → MySQL). See `PHASES.md` for the full objective/scope/completion criteria of each. This is the first phase of the now-fully-locked Phase 0–30 roadmap — work proceeds phase by phase from here.

## Next

The full roadmap (`PHASES.md`, Phase 0–30) is locked and gives the complete build order — Phase 1 (1a then 1b) is next up per Active above. A few items are still genuinely undecided even though most now have an assigned phase (the phase is *when* it gets decided, not that it already has been):
