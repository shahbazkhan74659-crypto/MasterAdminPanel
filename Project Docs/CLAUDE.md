...
- **The full build roadmap (Phase 0–30) is locked** — see `PHASES.md` and rule 3 above. "Spec-first vs. site-first" is resolved (site-first, by roadmap ordering — see `DECISIONS.md`). Remaining genuinely open items without a settled answer yet, even though most now have an assigned phase: exact secrets-storage mechanism ([Phase 10]), login mechanics ([Phase 4]), the SQL console's default write-safety posture ([Phase 8]), and versioning/history in the record editor (**not assigned to any phase** — a real, unscheduled gap). Do not treat any of these as decided; the phase assignment means *when* it gets decided, not that it already has been.
