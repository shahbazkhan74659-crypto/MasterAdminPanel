...
**Implemented:** `backend/` — Node.js (v24) + Express 4 + TypeScript, run via `tsx` in dev and compiled with `tsc` for production, ESM module format. DB access for the Phase 3 test endpoints uses `pg`, `mysql2`, and `better-sqlite3` (the same drivers the planned architecture locks in below), plus `dotenv` for loading `.env.local`. No frontend framework, Monaco, or Zustand wired in yet.
